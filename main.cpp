...
void sortasc(void)
{
        ...
        /// a and mini are to denote the position pointed at
        int a,b,mini;

        /// minv is to store the minimum value while sorting
        double minv;

        ///loop to scan through each value in the .txt file
        for(a = 0; a < valuesinfile; a++)
        {
            ///Letting mini equal to value of a
            mini=a;
            /// minv now equals the value in element indicated by mini
            minv=filevals[mini];

        ///for loop to compare a with other elements of the array
        for(b = a+1; b < valuesinfile; b++)
        {
        ///comparing two elements from the array to see which is smaller
        if(minv>filevals[b])
        {
            ///if smaller value is found then let minv be that small value
            minv=filevals[b];
            ///let mini indicator now equal to the value of b
            mini=b;
        }
        }
...
double maxv;    /// maxv is to store the maximum value while sorting
        ...
        for(a=0; a<valuesinfile; a++)
        {
            ...
            maxv=filevals[maxi];    /// maxv now equals the values in element indicated by maxi
